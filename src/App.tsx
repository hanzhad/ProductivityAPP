  ...
  return (
    <div class="flex flex-col min-h-screen">
      ...
      <Tabs value={currentView()} onChange={setCurrentView} class="flex-1 flex flex-col">
        <Tabs.List class="bg-white border-b border-gray-200">
          <div class="flex gap-1 px-4 overflow-x-auto">
            <Tabs.Trigger
              value="calendar"
              class="px-4 py-3 text-sm font-medium whitespace-nowrap transition-colors border-b-2 border-transparent text-gray-600 hover:text-gray-900 hover:border-gray-300 data-[selected]:border-primary data-[selected]:text-primary"
            >
              {t('tabs.calendar')}
            </Tabs.Trigger>
            <Tabs.Trigger
        ...
        <main class="flex-1 p-4 overflow-y-auto bg-gray-50">
          <Show when={error()}>
            <div class="mb-4 p-4 bg-red-50 border border-red-200 rounded-lg text-center">
              <p class="mb-3 text-red-800 font-medium">{error()}</p>
              <Button
                onClick={async () => {
                  const granted = await requestPermissions();
                  setPermissionsGranted(granted);
