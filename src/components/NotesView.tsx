  ...
  return (
    <div class="max-w-4xl mx-auto">
      <div class="flex justify-between items-center mb-6 flex-wrap gap-4">
        <h2 class="text-2xl font-semibold text-gray-900">{t('notes.title')}</h2>
        <Button
          onClick={loadNotes}
          disabled={loading()}
      ...
      <Show when={loading()}>
        <div class="flex flex-col items-center justify-center py-12 gap-4">
          ...
          <p class="text-gray-600">{t('notes.loadingNotes')}</p>
        </div>
      </Show>

      <Show when={!loading() && notes().length === 0}>
        <div class="bg-white rounded-lg shadow p-12 text-center">
          <p class="text-xl text-gray-500">{t('notes.noNotes')}</p>
        </div>
      </Show>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <For each={notes()}>
          {(note) => (
            <div class="bg-white rounded-lg shadow hover:shadow-lg transition-shadow p-5 active:scale-[0.98] flex flex-col min-h-[180px]">
              <h3 class="text-lg font-semibold text-gray-900 mb-3">{note.title}</h3>
              <div class="text-gray-600 text-sm leading-relaxed mb-4 flex-1 whitespace-pre-wrap">
                {note.content}
              </div>
